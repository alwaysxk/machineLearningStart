...
def loadData():
    ...
    for i, d in enumerate(f):
        ...
        if not d:
            continue
        d = map(float, d.strip().split())
        x.append([1.0, d[0], d[1]])
        y.append(d[2])
    x = array(x)
    y = array(y)
    print x
...
def gradAscent(train_features, train_labels):
    featuresMat = mat(train_features)
...
#     #dataMatrix = dataMatIn           #convert to NumPy matrix
...
def multiTest():
    ...
    for k in range(numTests):
        errorSum += colicTest()
    print "after %d iterations the average error rate is: %f" % (numTests, errorSum/float(numTest))
